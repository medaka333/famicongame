...
### M12 ―― グラフィック/音 ポリッシュ（祭り向け）
用途: 祭りで子供〜大人が遊ぶ → 見栄え・分かりやすさ重視。

- [x] 自機・敵の 2 フレームアニメ（噴射・動き）
- [x] 背景強化（星に色・サイズ・3 層奥行き）
- [x] 爆発の多フレーム化、パワーアップで自機/弾の見た目変化（M14）
- [ ] BGM 刷新（メロディ+ベース、ステージ毎）、SE 質向上、ボス BGM
- [ ] 祭り調整: 1 プレイ短縮・難度緩和・操作の常時表示・放置デモ

完了条件: 静止画から「動いて見える」画面、賑やかな音、初見でも遊べる導線。

### M13 ―― 祭りモード調整（子供/大人）
- [x] 難易度 2 モード KIDS / ADULT（タイトル選択）
  - KIDS: 残機5・敵弾0.6x・自機判定小(4px)・ボスHP0.55x・敵湧き緩め・2面
  - ADULT: 残機3・標準・自機判定8px・3面フル
- [x] タイトルでモード選択＋操作説明表示（放置でもアトラクト的）
- [x] プレイ中 HUD に操作（ARROW / J）常時表示
- [x] 1プレイ短縮（KIDS=2面・ザコ時間0.7x）

完了条件: タイトルで KIDS/ADULT を選べ、子供でもクリアでき、大人は歯ごたえあり。

### M15 ―― ボス強化・ステージ毎ボス
- [x] StageDef に boss_sprite、ステージ毎に別デザインのボス
- [x] ボス 3 種: boss1=要塞(紫) / boss2=蟹型(青) / boss3=最終(赤・巨大)
- [x] ボス毎に攻撃パターン差別化（boss1=要塞 aim/3way/radial、boss2=蟹 横ばらまき/扇、boss3=最終 radial12）
- [x] ボス撃破時に敵弾全消し（理不尽死防止）
- [ ] （将来）パーツ破壊、専用 HP ゲージ色

完了条件: 各ステージで見た目の違うボスが出る。
